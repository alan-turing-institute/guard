...
def test_imperial_density_summation(world_5x5):
    ...
    imperial_density = analysis.ImperialDensity(world,
                                                date_ranges=[date_range])

    # Ensure imperial density is zero with no large polities
    imperial_density.sample()
    assert np.all(
        imperial_density.data[date_range] == np.zeros([world.xdim, world.ydim])
        )
...
def test_population_data(generate_world):
    world = World.from_file(project_dir + '/test/data/test_map_5x5.yml')
    cities = analysis.CitiesPopulation(
        world, project_dir + '/test/data/test_cities.yml'
        )
