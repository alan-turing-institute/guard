...
def test_custom_parameter(default_parameters, custom_parameters):
    ...
    n_military_techs = 10
    ethnocide_max = 500
    custom = custom_parameters(n_military_techs=n_military_techs, ethnocide_max=ethnocide_max)

    print(default)
    print(parameters._default_parameters.items())
    assert default.n_military_techs == parameters._default_parameters['n_military_techs']
    assert custom.n_military_techs == n_military_techs
    assert default.n_military_techs != custom.n_military_techs

    assert default.ethnocide_max == parameters._default_parameters['ethnocide_max']
...
def test_invalid_key(custom_parameters):
    with pytest.raises(parameters.ParameterKeyException):
        params = custom_parameters(bad_key='test')
